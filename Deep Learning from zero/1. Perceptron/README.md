...
※なぜ、ステップ関数はニューラルネットワークで使えないのかというと、ステップ関数のようにほとんどの領域(0 以外の領域)で微分が0 となるような関数の場合、ニューラルネットワークの特徴である「学習」がうまく行えないからです。つまり、活性化関数がステップ関数以外なら何でも良いというわけではありません。「学習」と「関数の微分」の関係性についての詳細は後述します。
...
### 非線形関数
ニューラルネットワークではステップ関数のように微分が0 となるような活性化関数は使えないと言いましたが、その他にも明確に使用できない（使用する意味のない）関数があります。それは「**線形関数**」です。

線形関数とは*h(x) = cx* のような関数のことで、中間層のない1層のネットワークの場合、出力は*y = cx* となります。例えば、活性化関数が線形関数の層を3層重ねるようなネットワークを考えます。便宜的に各層の間の重みはすべて1 とすると、出力*y* は*h ( h ( h ( x ) ) ) = c × c × c × x* となります。ここで*a = c × c × c* とすると、*y = ax* と表すことができます。つまり、活性化関数の係数*c* の値を変化させることで、**多層でも単層でも出力結果を同じように表現できる**わけです。

したがって、活性化関数に線形関数を用いると、ニューラルネットワークの**層を深くするという利点を全く活かせない**ことになります。
