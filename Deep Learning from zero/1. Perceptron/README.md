...
線形関数とは*h(x) = cx* のような関数のことで、中間層のない1層のネットワークの場合（便宜的に各層の間の重みは1）、出力は*y = cx* となります。また、活性化関数が線形関数の層を3層重ねるようなネットワークの場合、出力*y* は*h ( h ( h ( x ) ) ) = c × c × c × x* となります。ここで*a = c × c × c* とすると、*y = ax* と表すことができます。つまり、活性化関数の係数*c* の値を変化させることで、**多層でも単層でも出力結果を同じように表現できる**わけです。
