...
線形関数とは*h(x)=cx* のような関数のことですが、例えば、活性化関数が線形関数の層を3層重ねるようなネットワークを考えます。便宜的に各層の間の重みはすべて1 とします。この場合は、出力*y* は*h(h(h(x))) = c × c × c × x* となります。ここで*a = c × c × c* とすると、*y = ax* と表すことができます。つまり、多層でも単層でも出力結果は同じように表現できるわけです。
